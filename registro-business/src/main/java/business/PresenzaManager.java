...
private static Logger log = Logger.getLogger("registro-business");
	...
	public static void aggiungiPresenza(Presenza p) {
		...
		Presenza pDb = null;
		if(p.getCodicePresenza() != 0) {
			pDb = em.find(Presenza.class, p.getCodicePresenza());
		}
		if (pDb == null) {
	...
	public static void aggiungiPresenza(Integer matricola, Date entrata, String codiceCorso, String note) {
		EntityManager em = JPAUtility.getEm();
		Registro r = em.find(Registro.class, codiceCorso);
		Studente s = em.find(Studente.class, matricola);
		if (r != null && s != null) {
	...
	public static void aggiungiPresenza(Integer matricola, Date entrata, Date uscita, String codiceCorso, String note) {
		EntityManager em = JPAUtility.getEm();
		Registro r = em.find(Registro.class, codiceCorso);
		Studente s = em.find(Studente.class, matricola);
		if (r != null && s != null) {
